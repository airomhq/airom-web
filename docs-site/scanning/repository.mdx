...
<Warning>
  The AIBOM's source block currently records the source kind and target only. Populating the git provenance block in the emitted document is not wired up yet, so do not expect `commit` or `dirty` to appear in the output today.
</Warning>
