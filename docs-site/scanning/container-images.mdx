...
`--ignore` works on image scans and matches against image-root paths. The `.gitignore`/`.airomignore` stacks do **not** apply here, because an image has no ignore files to read. Only your `--ignore` globs filter an image walk.
...
`site-packages`. So AIROM reads the installed metadata directly, from
...
followed by the component table (see [Output formats](/output/formats)). This is because
