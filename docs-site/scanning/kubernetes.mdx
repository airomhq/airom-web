...
A file that cannot be read, or a document that fails to parse, degrades to an `Unknown` and the walk continues, so one bad manifest never kills the run.
...
Images that could not be scanned are reported as warnings, and they do not trip the gate. Given the resolution limitation above, do not treat a green `k8s --manifests` gate as evidence that your workload images are clean.
